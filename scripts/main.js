(async function() {
  ...
  const [ playersMap, rosters, users ] = await Promise.all([
    ...
    fetch('https://api.sleeper.app/v1/league/1226594146450423808/rosters').then(r => r.json()),
    fetch('https://api.sleeper.app/v1/league/1226594146450423808/users').then(r => r.json()),
  ]);
  ...
  users.forEach(u => {
    ...
    const name = u.metadata?.team_name
      || u.display_name    // fallback if no metadata
      || `Owner ${u.user_id}`;
    teamNameByOwner[u.user_id] = name;
  });

  const posOrder = ["QB","RB","WR","TE","DB","DE","LB","K"];
  ...
  rosters.forEach(r => {
    ...
    const teamName = teamNameByOwner[r.owner_id]
      || `Roster ${r.roster_id}`;
    ...
    const sortedIds = [...r.players].sort((a, b) => {
      ...
      const iA = posOrder.indexOf(posA) >= 0 ? posOrder.indexOf(posA) : posOrder.length;
      const iB = posOrder.indexOf(posB) >= 0 ? posOrder.indexOf(posB) : posOrder.length;
